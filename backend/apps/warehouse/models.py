...
class WarehouseProduct(models.Model):
    ...
    product_type = models.CharField(
        "产品类型", max_length=20, help_text="木门/合金门/防盗门", db_index=True
    )
...
class HardwareInventory(models.Model):
    ...
    hardware_type = models.CharField(
        "类型", max_length=20, help_text="木门/合金门/防盗门用"
    )
...
class PendingGoods(models.Model):
    ...
    door_type = models.CharField(
        "门种类", max_length=20, help_text="木门/合金门/防盗门"
    )
    spec_size = models.JSONField(
        "规格尺寸", default=dict, blank=True, help_text="详细拆分"
    )
    ...
    inbound_reason = models.CharField(
        "入库原因", max_length=200, blank=True, default=""
    )
    ...
    matched_order_no = models.CharField(
        "匹配订单号", max_length=50, blank=True, default=""
    )
    city = models.CharField("城市", max_length=50, blank=True, default="")
    created_at = models.DateTimeField("创建时间", auto_now_add=True)
    updated_at = models.DateTimeField("更新时间", auto_now=True)
...
class StockRecord(models.Model):
    ...
    record_type = models.CharField(
        "记录类型", max_length=10, choices=RECORD_TYPE_CHOICES
    )
    ...
    related_task_id = models.CharField(
        "关联任务ID", max_length=50, blank=True, default=""
    )
