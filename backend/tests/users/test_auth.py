...
class TestUserAuthentication:
    ...
    def test_user_registration_decoration(self, api_client):
        """测试装企用户注册"""
        response = api_client.post('/api/v1/users/register/decoration/', data={
            'phone': '13900139000',
            'password': 'Test123456',
            'contact_name': '装企注册用户',
        })
        assert response.status_code == 201, f"装企注册失败: {response.status_code} - {response.data}"
    ...
    def test_user_registration_staff(self, api_client):
        """测试管理人员注册（待审核）"""
        response = api_client.post('/api/v1/users/register/staff/', data={
            'phone': '13900139001',
            'password': 'Test123456',
            'real_name': '管理人员注册',
            'user_type': 'management',
        })
        assert response.status_code == 201, f"注册失败: {response.status_code} - {response.data}"
    ...
    def test_user_login(self, api_client, create_user, user_data):
        ...
        assert response.status_code == 200, f"登录失败: {response.status_code} - {response.data}"
